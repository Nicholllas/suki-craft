...
@section('content')
    ...
    <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        <div class="flex items-end justify-between gap-4 border-b border-stone-200 pb-6">
            <div>
                <p class="text-xs font-bold uppercase tracking-[0.2em] text-rose-500">Pesananmu</p>
                <h1 class="mt-2 font-serif text-4xl font-semibold tracking-tight text-stone-800">Keranjang belanja</h1>
            </div>
        ...
        @if ($items->isEmpty())
            <div class="mt-8 rounded-3xl border border-dashed border-rose-200 bg-rose-50/60 px-6 py-16 text-center">
                ...
                <h2 class="mt-5 font-serif text-2xl font-semibold text-stone-800">Keranjangmu masih kosong</h2>
                <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-stone-500">Jelajahi koleksi bunga segar kami dan temukan buket untuk momen spesialmu.</p>
                <a href="{{ route('products.index') }}" class="mt-6 inline-flex rounded-full bg-rose-500 px-5 py-3 text-sm font-semibold text-white transition hover:bg-rose-600">Jelajahi koleksi buket</a>
            </div>
        @else
            <div class="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start">
                <div class="space-y-4">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex gap-3">
                                        ...
                                        <form method="POST" action="{{ route('cart.remove', $item->id) }}" data-confirm="Buket ini akan dihapus dari keranjang." data-confirm-button="Ya, hapus" data-confirm-title="Hapus buket dari keranjang?">
                                            @csrf
                                            @method('DELETE')
                                            <button class="grid h-9 w-9 place-items-center rounded-full text-stone-400 transition hover:bg-rose-50 hover:text-rose-600" aria-label="Hapus {{ $item->product->name }}">
                                                <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="m6 7 .75 12h10.5L18 7M9.5 10.5v5m5-5v5M9 7V4.5h6V7M4.5 7h15" /></svg>
                                            </button>
                                        </form>
                                    </div>

                                    @if ($item->card_message || $item->special_note)
                                        <div class="mt-3 space-y-1 rounded-xl bg-stone-50 px-3 py-2 text-xs leading-5 text-stone-500">
                                            @if ($item->card_message)
                                                <p><span class="font-semibold text-stone-600">Kartu:</span> {{ $item->card_message }}</p>
                                            @endif
                                            @if ($item->special_note)
                                                <p><span class="font-semibold text-stone-600">Catatan:</span> {{ $item->special_note }}</p>
                                            @endif
                                        </div>
                                    @endif
                    ...
                    @endforeach
                    <a href="{{ route('products.index') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-rose-600 hover:text-rose-700">
                        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="m19 12H5m5-5-5 5 5 5" /></svg>
                        Tambah buket lain
                    </a>
                </div>
                ...
                <aside data-cart-summary class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm lg:sticky lg:top-24 lg:shadow-xl lg:shadow-stone-900/5">
                    ...
                    <dl class="mt-5 space-y-3 text-sm">
                        <div class="flex items-center justify-between gap-4 border-b border-stone-100 pb-4 text-stone-600">
                            <dt>Subtotal buket ({{ $items->sum('bundle_quantity') }} buket)</dt>
                            <dd class="font-semibold text-stone-800">Rp{{ number_format($total, 0, ',', '.') }}</dd>
                        </div>
                        <div class="text-xs leading-5 text-stone-500">
