        ...
        <nav class="flex-1 overflow-y-auto px-4 py-5">
            ...
            <div class="space-y-1">
                ...
                <x-admin.nav-link :href="route('admin.orders.index')" :active="request()->routeIs('admin.orders.*')">
                    ...
                    Semua Pesanan
                </x-admin.nav-link>                <x-admin.nav-link :href="route('admin.custom-requests.index')" :active="request()->routeIs('admin.custom-requests.*')">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21.75s-7.5-4.37-7.5-10.12A4.13 4.13 0 0 1 12 9.2a4.13 4.13 0 0 1 7.5 2.43C19.5 17.38 12 21.75 12 21.75Z" /></svg>
                    Custom Bouquet
                    @if ($pendingCustomRequestCount > 0)
                        <span class="ml-auto rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-bold text-rose-700">{{ $pendingCustomRequestCount }}</span>
                    @endif
                </x-admin.nav-link>
            ...
            <div class="space-y-1">
                ...
                <x-admin.nav-link :href="route('admin.promo-banners.index')" :active="request()->routeIs('admin.promo-banners.*')"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3.75" y="5.25" width="16.5" height="13.5" rx="1.5" /><path stroke-linecap="round" stroke-linejoin="round" d="m6.75 15 3.75-3.75 2.25 2.25 1.5-1.5 3 3" /></svg>Banner Promo</x-admin.nav-link>
                <x-admin.nav-link :href="route('admin.reviews.index')" :active="request()->routeIs('admin.reviews.*')"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 12h7.5m-7.5 3h4.5M5.25 4.5h13.5a1.5 1.5 0 0 1 1.5 1.5v9a1.5 1.5 0 0 1-1.5 1.5H12l-3.75 3v-3H5.25a1.5 1.5 0 0 1-1.5-1.5V6a1.5 1.5 0 0 1 1.5-1.5Z" /></svg>Ulasan @if($pendingReviewCount > 0)<span class="ml-auto rounded-full bg-rose-100 px-2 py-0.5 text-[10px] font-bold text-rose-700">{{ $pendingReviewCount }}</span>@endif</x-admin.nav-link>
            </div>
            ...
            <div class="flex h-20 items-center gap-3 px-4 sm:px-6 lg:px-8">
                ...
                <div class="ml-auto flex items-center gap-2 sm:gap-3">
                    ...
                    <div class="relative" @click.outside="notificationsOpen = false">
                        <button @click="notificationsOpen = !notificationsOpen" :aria-expanded="notificationsOpen" class="relative grid h-11 w-11 place-items-center rounded-xl border border-stone-200 bg-white text-stone-600 transition hover:border-rose-200 hover:text-rose-600" aria-label="Notifikasi">
                            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M14.25 18.75h-4.5m9-2.25H5.25l1.5-2.25v-4.5a5.25 5.25 0 0 1 10.5 0v4.5l1.5 2.25Z" /></svg>
                            @if ($adminNotificationCount > 0)
                                <span class="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white"></span>
                            @endif
                        </button>
                        <div x-cloak x-show="notificationsOpen" x-transition.origin.top.right class="absolute right-0 mt-3 w-80 overflow-hidden rounded-2xl border border-stone-100 bg-white shadow-xl shadow-stone-900/10">
                            <div class="flex items-center justify-between border-b border-stone-100 px-4 py-3">
                                ...
                                @if ($adminNotificationCount > 0)
                                    <span class="rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-bold text-rose-600">{{ $adminNotificationCount }} perlu tindakan</span>
                                @endif
                            </div>
                            <div class="space-y-2 p-3">
                                @if ($pendingPaymentVerificationCount > 0)
                                    <a href="{{ route('admin.payment-verifications.index') }}" class="block rounded-xl bg-amber-50 p-3 text-sm transition hover:bg-amber-100"><p class="font-semibold text-amber-800">{{ $pendingPaymentVerificationCount }} pembayaran menunggu</p><p class="mt-1 text-xs leading-5 text-amber-700">Tinjau bukti pembayaran pelanggan yang baru masuk.</p></a>
                                @endif
                                @if ($pendingCustomRequestCount > 0)
                                    <a href="{{ route('admin.custom-requests.index') }}" class="block rounded-xl bg-rose-50 p-3 text-sm transition hover:bg-rose-100"><p class="font-semibold text-rose-800">{{ $pendingCustomRequestCount }} Custom Bouquet menunggu</p><p class="mt-1 text-xs leading-5 text-rose-700">Ada permintaan baru atau revisi pelanggan yang perlu ditinjau.</p></a>
                                @endif
                                @if ($adminNotificationCount === 0)
                                    <div class="rounded-xl bg-stone-50 p-3 text-sm"><p class="font-semibold text-stone-700">Tidak ada tindakan baru</p><p class="mt-1 text-xs leading-5 text-stone-500">Pembayaran dan Custom Bouquet yang perlu ditinjau akan muncul di sini.</p></div>
                                @endif
                            </div>
                        </div>
                    </div>
