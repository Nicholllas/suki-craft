...
<head>
    ...
    <meta name="description" content="@yield('description', 'Sukicraft.id - Flower & Gift Shop')">
    <link rel="canonical" href="{{ url()->current() }}">
    <meta property="og:title" content="@yield('title', 'Sukicraft.id')">
    <meta property="og:description" content="@yield('description', 'Sukicraft.id - Flower & Gift Shop')">
...
<body class="min-h-screen overflow-x-hidden bg-[#fffdf9] font-sans text-stone-800 antialiased">
    ...
    <header x-data="{ accountOpen: false, open: false, cartCount: {{ $cartItemCount }} }" @cart-updated.window="cartCount = $event.detail.count ?? 0" class="sticky top-0 z-50 border-b border-stone-100 bg-[#fffdf9]/95 backdrop-blur">
        <div class="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            <a href="{{ route('home') }}" class="group flex items-center gap-2.5" aria-label="Sukicraft.id beranda">
                ...
                <span class="leading-none"><span class="block font-serif text-xl font-semibold tracking-tight text-stone-800">Sukicraft.id</span><span class="mt-1 block text-[9px] font-bold uppercase tracking-[0.23em] text-rose-500">Bouquet, Mahar & Seserahan</span></span>
            </a>
    ...
    <footer class="bg-stone-900 text-stone-300">
        <div class="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-2 lg:grid-cols-4 lg:px-8">
            <div class="lg:col-span-2"><a href="{{ route('home') }}" class="font-serif text-2xl font-semibold text-white">Sukicraft.id</a><p class="mt-4 max-w-sm text-sm leading-6 text-stone-400">Buket bunga yang dirangkai dengan penuh rasa, untuk membantu Anda menyampaikan hal yang tak selalu mudah diucapkan.</p></div>
            <div><h2 class="text-sm font-semibold text-white">Jelajahi</h2><ul class="mt-4 space-y-3 text-sm"><li><a href="{{ route('products.index') }}" class="transition hover:text-rose-300">Koleksi Buket</a></li><li><a href="{{ route('tracking.create') }}" class="transition hover:text-rose-300">Lacak Pesanan</a></li><li><a href="{{ route('about') }}" class="transition hover:text-rose-300">Tentang Sukicraft.id</a></li><li><a href="{{ route('how_to_order') }}" class="transition hover:text-rose-300">Cara Pemesanan</a></li></ul></div>
        ...
        <div class="border-t border-stone-800 px-4 py-5 text-center text-xs text-stone-500">&copy; {{ date('Y') }} Sukicraft.id. Made with love.</div>
    </footer>
</body>
</html>
