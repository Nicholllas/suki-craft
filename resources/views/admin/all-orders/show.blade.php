        ...
        <div class="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
            ...
            <aside class="space-y-6">
                <x-card padding="p-5">
                    ...
                    <div class="mt-4 space-y-3">
                        ...
                        @if ($order->biteship_tracking_id)
                            <form method="POST" action="{{ route('admin.deliveries.biteship.sync', $order) }}">@csrf<button type="submit" class="inline-flex h-11 w-full items-center justify-center rounded-xl border border-sky-200 bg-white px-4 text-sm font-semibold text-sky-700 transition hover:bg-sky-50">Cek status pengiriman</button></form>
                        @endif
                ...
                <x-card padding="p-5">
                    ...
                    <div class="mt-5 border-t border-stone-100 pt-5"><p class="text-xs font-semibold text-stone-400">Kurir</p>@if (in_array($order->courier_company, config('biteship.couriers', []), true))<p class="mt-1 text-sm font-semibold text-stone-800">{{ config('biteship.courier_names.'.$order->courier_company, str($order->courier_company)->upper()) }} · {{ str($order->courier_service)->upper() }}</p>@elseif ($order->courier)<p class="mt-1 text-sm font-semibold text-stone-800">{{ $order->courier->name }}</p><p class="mt-1 text-sm text-stone-500">{{ $order->courier->phone }}</p>@else<p class="mt-1 text-sm font-semibold text-stone-800">Belum ditugaskan</p>@endif</div>
                    @if ($order->tracking_number)<div class="mt-5 border-t border-stone-100 pt-5"><p class="text-xs font-semibold text-stone-400">Nomor resi</p><p class="mt-1 font-mono text-sm font-semibold text-stone-800">{{ $order->tracking_number }}</p>@if ($order->tracking_url)<a href="{{ $order->tracking_url }}" target="_blank" rel="noopener noreferrer" class="mt-3 inline-flex text-sm font-semibold text-rose-600 transition hover:text-rose-700">Buka tracking ekspedisi ↗</a>@endif</div>@endif
