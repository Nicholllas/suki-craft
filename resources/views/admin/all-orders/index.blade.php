            ...
            <x-data-table :headers="['Pesanan', 'Pelanggan', 'Dibuat', 'Pengiriman', 'Total', 'Status', 'Aksi']" :is-empty="$orders->isEmpty()" empty-title="Pesanan tidak ditemukan" empty-description="Ubah atau reset filter untuk melihat pesanan yang tersedia.">
                @foreach ($orders as $order)
                    <tr>
                        ...
                        <td class="px-5 py-4"><p class="whitespace-nowrap text-sm font-medium text-stone-700">{{ $order->delivery_date->locale('id')->translatedFormat('d M Y') }}</p><p class="mt-0.5 whitespace-nowrap text-xs text-stone-400">{{ config('delivery.time_slots.'.$order->delivery_time_slot.'.label', $order->delivery_time_slot) }}</p></td>
