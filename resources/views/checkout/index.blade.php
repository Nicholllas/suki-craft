...
@section('content')
    <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        <form method="POST" action="{{ route('checkout.store') }}" class="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_23rem] lg:items-start" x-data='checkoutShipping(@js([
            "areasUrl" => route("checkout.shipping-areas.search"),
            "csrfToken" => csrf_token(),
            "deliveryFee" => $deliveryFee,
            "initialSelection" => $shippingSelection,
            "promotionCode" => old("promotion_code", session("checkout.promotion_code")),
            "promotionUrl" => route("checkout.promotions.validate"),
            "ratesUrl" => route("checkout.shipping-rates.index"),
            "selectionUrl" => route("checkout.shipping-option.store"),
        ]))'>
            @csrf
            ...
            <div class="space-y-6">
                ...
                <section class="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm sm:p-7">
                    ...
                    <div class="mt-6 grid gap-5 sm:grid-cols-2">
                        ...
                        <div class="relative sm:col-span-2">
                            <label for="delivery-area" class="text-sm font-semibold text-stone-700">Kecamatan tujuan</label>
                            <input id="delivery-area" x-model="areaQuery" type="text" autocomplete="off" class="mt-2 h-12 w-full rounded-xl border-stone-200 px-4 text-sm text-stone-800 placeholder:text-stone-400 focus:border-rose-300 focus:ring-rose-200" placeholder="Cari kecamatan atau kota tujuan">
                            <p class="mt-2 text-xs text-stone-400">Pilih area dari daftar agar tarif ekspedisi dihitung akurat.</p>
                            <p x-cloak x-show="areaLoading" class="mt-2 text-xs font-medium text-stone-500">Mencari area...</p>
                            <p x-cloak x-show="areaError" x-text="areaError" class="mt-2 text-xs font-medium text-rose-600"></p>
                            <div x-cloak x-show="areas.length > 0" class="absolute z-20 mt-2 max-h-64 w-full overflow-y-auto rounded-2xl border border-stone-200 bg-white p-2 shadow-xl shadow-stone-900/10">
                                <template x-for="area in areas" :key="area.id">
                                    <button type="button" x-on:click="selectArea(area)" class="block w-full rounded-xl px-3 py-3 text-left text-sm transition hover:bg-rose-50"><span x-text="area.name" class="block font-semibold text-stone-700"></span><span x-show="area.postal_code" x-text="`Kode pos ${area.postal_code}`" class="mt-1 block text-xs text-stone-400"></span></button>
                                </template>
                            </div>
                        </div>
                        <div class="sm:col-span-2">
                ...
                <section x-cloak x-show="selectedArea || ratesLoading || rates.length > 0 || ratesError" class="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm sm:p-7">
                    <div class="flex items-start gap-3"><span class="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-rose-50 text-sm font-bold text-rose-600">3</span><div><h2 class="font-serif text-2xl font-semibold text-stone-800">Pilih ekspedisi</h2><p class="mt-1 text-sm leading-6 text-stone-500">Tarif dihitung dari berat buket dan kecamatan tujuan.</p></div></div>
                    <p x-cloak x-show="ratesLoading" class="mt-6 rounded-2xl bg-stone-50 px-4 py-3 text-sm text-stone-600">Menghitung ongkir ekspedisi...</p>
                    <p x-cloak x-show="ratesFallback" class="mt-6 rounded-2xl bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">Tarif ekspedisi sedang tidak tersedia. Kami menggunakan tarif pengiriman standar agar checkout tetap dapat dilanjutkan.</p>
                    <p x-cloak x-show="ratesError" x-text="ratesError" class="mt-6 rounded-2xl bg-rose-50 px-4 py-3 text-sm leading-6 text-rose-700"></p>
                    <div x-cloak x-show="rates.length > 0" class="mt-6 grid gap-3">
                        <template x-for="rate in rates" :key="`${rate.company}-${rate.service}`">
                            <button type="button" x-on:click="selectRate(rate)" :disabled="selectionLoading" :class="shippingOption?.courier_company === rate.company && shippingOption?.courier_service === rate.service ? 'border-rose-400 bg-rose-50 ring-2 ring-rose-100' : 'border-stone-200 bg-white hover:border-rose-200 hover:bg-rose-50/40'" class="grid gap-2 rounded-2xl border p-4 text-left transition disabled:cursor-wait disabled:opacity-60 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><span><span x-text="rate.courier" class="block text-sm font-bold text-stone-800"></span><span class="mt-1 block text-xs text-stone-500"><span x-text="rate.service_name"></span><span x-show="rate.estimated_days" x-text="` · ${rate.estimated_days}`"></span></span></span><span class="text-sm font-bold text-rose-600">Rp<span x-text="format(rate.price)"></span></span></button>
                        </template>
                    </div>
                </section>
            </div>

            <aside class="sticky bottom-3 rounded-3xl border border-stone-200 bg-white p-5 shadow-xl shadow-stone-900/5 lg:top-24 lg:bottom-auto lg:p-6">
                ...
                <dl class="mt-5 space-y-3 text-sm">
                    ...
                    <div class="flex items-center justify-between text-stone-500"><dt>Biaya pengiriman</dt><dd x-cloak x-show="shippingOption" class="font-medium text-stone-800">Rp<span x-text="format(shippingFee)"></span></dd><dd x-show="! shippingOption" class="font-medium text-stone-400">Pilih tarif</dd></div>
                    ...
                    <div class="flex items-end justify-between border-t border-stone-100 pt-4"><dt class="font-semibold text-stone-800">Total pembayaran</dt><dd class="font-serif text-2xl font-semibold text-stone-800">Rp<span x-text="format({{ $subtotal }} + shippingFee - discount)">{{ number_format($subtotal + $deliveryFee, 0, ',', '.') }}</span></dd></div>
                </dl>

                @error('shipping')<p class="mt-4 rounded-xl bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700">{{ $message }}</p>@enderror
                <button type="submit" :disabled="! shippingOption || selectionLoading" class="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-rose-500 px-5 text-sm font-semibold text-white shadow-lg shadow-rose-200 transition hover:bg-rose-600 focus:outline-none focus:ring-2 focus:ring-rose-300 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60">
                    Buat pesanan
