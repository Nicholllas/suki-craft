...
@section('content')
    <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        <form method="POST" action="{{ route('checkout.store') }}" class="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_23rem] lg:items-start">
            ...
            <aside class="sticky bottom-3 rounded-3xl border border-stone-200 bg-white p-5 shadow-xl shadow-stone-900/5 lg:top-24 lg:bottom-auto lg:p-6" x-data="{ showItems: window.innerWidth >= 1024, code: '{{ old('promotion_code', session('checkout.promotion_code')) }}', discount: 0, error: '', loading: false, format(value) { return new Intl.NumberFormat('id-ID').format(value) }, async applyPromotion() { this.loading = true; this.error = ''; const response = await fetch('{{ route('checkout.promotions.validate') }}', { method: 'POST', headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: JSON.stringify({ code: this.code, customer_phone: document.getElementById('customer-phone').value }) }); const data = await response.json(); this.loading = false; if (! response.ok) { this.discount = 0; this.error = data.errors?.promotion_code?.[0] ?? data.message; return; } this.code = data.code; this.discount = data.discount_amount; } }">
