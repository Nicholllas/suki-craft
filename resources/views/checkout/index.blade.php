        ...
        <form method="POST" action="{{ route('checkout.store') }}" class="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_23rem] lg:items-start">
            ...
            <aside data-checkout-summary class="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm lg:sticky lg:top-24 lg:p-6 lg:shadow-xl lg:shadow-stone-900/5" x-data="{ showItems: window.innerWidth >= 1024, code: '{{ old('promotion_code', session('checkout.promotion_code')) }}', discount: 0, error: '', loading: false, format(value) { return new Intl.NumberFormat('id-ID').format(value) }, async applyPromotion() { this.loading = true; this.error = ''; try { const response = await fetch('{{ route('checkout.promotions.validate') }}', { method: 'POST', credentials: 'same-origin', headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'X-Requested-With': 'XMLHttpRequest' }, body: JSON.stringify({ code: this.code, customer_phone: document.getElementById('customer-phone').value }) }); const payload = await response.text(); const data = payload ? JSON.parse(payload) : {}; if (! response.ok) { this.discount = 0; this.error = data.errors?.promotion_code?.[0] ?? data.message ?? 'Kode promo tidak dapat diterapkan. Silakan coba lagi.'; return; } this.code = data.code; this.discount = data.discount_amount; } catch (error) { this.discount = 0; this.error = 'Kode promo tidak dapat diterapkan. Muat ulang halaman lalu coba kembali.'; } finally { this.loading = false; } } }">
                ...
                <dl class="mt-5 space-y-3 text-sm">
                    ...
                    <div class="rounded-xl bg-rose-50 p-3"><label for="promotion-code" class="text-xs font-semibold text-stone-700">{{ __('storefront.checkout.promo_code') }}</label><div class="mt-2 flex gap-2"><input id="promotion-code" name="promotion_code" x-model="code" class="min-w-0 flex-1 rounded-lg border-stone-200 px-3 py-2 text-sm uppercase focus:border-rose-300 focus:ring-rose-200" placeholder="PROMO2026"><button type="button" @click="applyPromotion" :disabled="loading" class="rounded-lg bg-stone-800 px-3 text-xs font-semibold text-white disabled:opacity-60" x-text="loading ? '...' : '{{ __('storefront.checkout.apply') }}'"></button></div><p x-show="error" x-text="error" class="mt-2 text-xs text-rose-600"></p>@error('promotion_code')<p class="mt-2 text-xs text-rose-600">{{ $message }}</p>@enderror</div>
                    <div x-show="discount > 0" class="flex items-center justify-between text-emerald-600"><dt>{{ __('storefront.checkout.promo_discount') }}</dt><dd class="font-semibold">-Rp<span x-text="format(discount)"></span></dd></div>
                    <div class="flex items-end justify-between border-t border-stone-100 pt-4"><dt class="font-semibold text-stone-800">{{ __('storefront.checkout.total') }}</dt><dd class="font-serif text-2xl font-semibold text-stone-800">Rp<span x-text="format({{ $subtotal + $deliveryFee }} - discount)">{{ number_format($subtotal + $deliveryFee, 0, ',', '.') }}</span></dd></div>
                </dl>
