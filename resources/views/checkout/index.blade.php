...
@section('content')
    <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        <form method="POST" action="{{ route('checkout.store') }}" class="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_23rem] lg:items-start">
            ...
            <aside class="sticky bottom-3 rounded-3xl border border-stone-200 bg-white p-5 shadow-xl shadow-stone-900/5 lg:top-24 lg:bottom-auto lg:p-6" x-data="{ showItems: window.innerWidth >= 1024 }">
                ...
                <dl class="mt-5 space-y-3 text-sm">
                    ...
                    <div class="flex items-end justify-between border-t border-stone-100 pt-4"><dt class="font-semibold text-stone-800">Total pembayaran</dt><dd class="font-serif text-2xl font-semibold text-stone-800">Rp{{ number_format($subtotal + $deliveryFee, 0, ',', '.') }}</dd></div>
                </dl>
