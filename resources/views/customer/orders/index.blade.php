    ...
    <section class="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        <div class="mt-6 space-y-4">
            @forelse($orders as $order)
                <a href="{{ route('customer.orders.show', $order) }}" class="block rounded-2xl border border-stone-200 bg-white p-5 shadow-sm transition hover:border-rose-200 hover:shadow-md"><div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p class="font-mono text-sm font-bold tracking-wide text-stone-800">{{ $order->order_number }}</p><p class="mt-2 text-sm text-stone-500">{{ $order->delivery_date->translatedFormat('d F Y') }} · {{ config('delivery.time_slots.'.$order->delivery_time_slot, $order->delivery_time_slot) }}</p><p class="mt-1 text-xs text-stone-400">Dibuat {{ $order->created_at->locale('id')->translatedFormat('d F Y, H.i') }} WIB</p></div><div class="flex items-center justify-between gap-4 sm:justify-end"><x-status-badge :status="$order->status->label()" /><span class="text-sm font-semibold text-rose-600">Detail →</span></div></div></a>
            @empty
