...
@section('title', __('storefront.account_pages.orders.title'))

@section('content')
    <section class="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        <div>
            <p class="text-xs font-bold uppercase tracking-[0.18em] text-rose-500">{{ __('storefront.account_pages.account') }}</p>
            <h1 class="mt-2 font-serif text-3xl font-semibold tracking-tight text-stone-800">{{ __('storefront.account_pages.orders.heading') }}</h1>
            <p class="mt-2 text-sm text-stone-500">{{ __('storefront.account_pages.orders.intro') }}</p>
        </div>
        ...
        <div class="mt-6 space-y-4">
                ...
                <article class="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-sm text-stone-500">{{ __('storefront.account_pages.orders.delivery', ['date' => $order->delivery_date->locale(app()->getLocale())->translatedFormat('d F Y'), 'time' => __('delivery.slots.'.$order->delivery_time_slot)]) }}</p>
                            <p class="mt-1 text-xs text-stone-400">{{ __('storefront.account_pages.orders.created', ['date' => $order->created_at->locale(app()->getLocale())->translatedFormat('d F Y, H.i').' WIB']) }}</p>
                            <p class="mt-3 text-sm font-semibold text-stone-800">{{ __('storefront.account_pages.orders.total') }} <span class="ml-2">Rp{{ number_format($order->total, 0, ',', '.') }}</span></p>
                        </div>
            ...
            @empty
                <div class="rounded-3xl border border-dashed border-rose-200 bg-rose-50/60 px-6 py-14 text-center">
                    ...
                    <h2 class="mt-5 font-serif text-2xl font-semibold text-stone-800">{{ __('storefront.account_pages.orders.empty_heading') }}</h2>
                    <p class="mx-auto mt-2 max-w-md text-sm leading-6 text-stone-500">{{ __('storefront.account_pages.orders.empty_description') }}</p>
                    <a href="{{ route('products.index') }}" class="mt-6 inline-flex rounded-full bg-rose-500 px-5 py-3 text-sm font-semibold text-white transition hover:bg-rose-600">{{ __('storefront.account_pages.orders.browse') }}</a>
                </div>
            @endforelse
        </div>
