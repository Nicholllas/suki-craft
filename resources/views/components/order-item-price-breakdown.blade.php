...
<dl class="mt-3 space-y-2 rounded-xl bg-stone-50 px-3 py-3 text-xs leading-5 text-stone-600">
    <div class="flex items-start justify-between gap-4">
        <dt>
            {{ $item->custom_request_id ? 'Harga Custom Bouquet' : 'Biaya jasa merangkai' }}
            <span class="text-stone-400">per buket</span>
        </dt>
    ...
    @if ($item->custom_request_id)
        <div class="flex items-start justify-between gap-4">
            <dt>Rincian isi</dt>
            <dd class="shrink-0 text-stone-500">Sesuai permintaan yang disetujui</dd>
        </div>
    @else
