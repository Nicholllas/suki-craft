...
<nav class="grid gap-1 rounded-2xl border border-stone-200 bg-white p-2 text-sm font-semibold text-stone-600 sm:flex sm:items-center" aria-label="Navigasi akun">
    ...
    <a href="{{ route('customer.custom-requests.index') }}" @class(['rounded-xl px-4 py-3 transition hover:bg-rose-50 hover:text-rose-600', 'bg-rose-50 text-rose-600' => $active === 'custom-requests'])>Custom Bouquet</a>
