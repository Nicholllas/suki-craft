...
@section('content')
    ...
    <section id="cara-pesan" class="scroll-mt-20 bg-white py-20 sm:py-24"><div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div class="mx-auto max-w-2xl text-center"><p class="text-xs font-bold uppercase tracking-[0.2em] text-rose-500">Mudah & menyenangkan</p><h2 class="mt-3 font-serif text-4xl font-semibold tracking-tight text-stone-800 sm:text-5xl">Kirim bahagia dalam tiga langkah.</h2></div><div class="relative mt-12 grid gap-8 md:grid-cols-3"><div class="absolute left-[16%] right-[16%] top-9 hidden border-t border-dashed border-rose-200 md:block"></div>@foreach ([['01', 'Pilih rangkaian', 'Temukan buket yang paling mewakili perasaan Anda.'], ['02', 'Tulis pesan', 'Tambahkan kartu ucapan untuk sentuhan yang lebih personal.'], ['03', 'Kirimkan cinta', 'Pesanan Anda siap menjadi kejutan yang berkesan.']] as $step)<div class="relative text-center"><span class="mx-auto grid h-[72px] w-[72px] place-items-center rounded-full bg-rose-50 font-serif text-xl font-semibold text-rose-500 ring-8 ring-white">{{ $step[0] }}</span><h3 class="mt-5 text-lg font-semibold text-stone-800">{{ $step[1] }}</h3><p class="mx-auto mt-2 max-w-xs text-sm leading-6 text-stone-500">{{ $step[2] }}</p></div>@endforeach</div></div></section>
